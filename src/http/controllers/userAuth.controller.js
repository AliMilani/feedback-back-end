 ...
 response({
  ...
  async login ({ response, body, header }) {
    ...
    try {
      ...
      if (error.message === "Invalid password") { return response({ code: "LOGIN_FAILED" }) }
      if (error.message === "User not found") { return response({ code: "USER_NOT_FOUND" }) } // todo: rate limit - to prevent brute force / user enumeration attacks
      throw error
    }
  }
