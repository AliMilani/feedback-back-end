...
class RouteBuilder {
  ...
  #validatePath (path) {
    ...
    if (path.length > 1 && path[path.length - 1] === "/") { throw new Error("path should not end with / when not root") }
  }
  ...
  #validateMethod (method) {
    ...
    if (!allowedMethods.includes(method)) { throw new Error("method should be one of get, post, put, delete") }
  }
  ...
  setUserRole (userRole) {
    ...
    } else if (userRole === userRoles.USER) { this.#userAuthMiddleware = userAuthMiddleware }
    return this
  }
